...
### ⚠️ 大盤 regime 相依（重要 caveat）

上面「贏家沒籌碼、籌碼配角」的結論**來自單一 regime**：5/26→7/14 這段**全市場等權只 +0.8%（近持平）、但資金極度集中在少數熱門題材**（功率 +23%）——是「**盤整/輪動、risk-on 分散市**」，不是全面多頭、也**完全沒有空頭樣本**。

- **「籌碼 vs 動能」的權重很可能隨 regime 變**：risk-on/輪動市散戶動能主導（籌碼可缺席）；**risk-off/廣泛下跌時，法人/大戶支撐（籌碼）可能反而更重要**（當防禦濾網、避免追到崩跌的散戶股）。
- **接口**：已做好的**大盤分級儀表板**（`classify_market_regime`，方向 + 資金集中度）就是現成的 regime 輸入，之後可把進貨分/權重做成 **regime-aware**。
- **現況無法校準**：籌碼資料全在這一種 regime（法人 52 天、大戶 7 週）→ 得等資料橫跨多 regime 才能驗「籌碼在空頭是否更值錢」。回測也必須**按 regime 分段**（見 backtest spec）。
