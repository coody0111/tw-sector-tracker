...
獨立「持股健檢」頁面/區塊（比照 `chips.html` 的做法），不疊加進現有 Top10／分組卡片。理由：`DESIGN.md` 已定過「不重複資訊」「不塞爆卡片」的規則，均線健檢資訊量（4條均線+排列狀態+RS+五級分類）不適合再塞進已經很滿的卡片。

具體頁面設計（區塊順序、視覺呈現）留到下一階段 UI 設計時再細化，不在本次資料層 spec 範圍內。
