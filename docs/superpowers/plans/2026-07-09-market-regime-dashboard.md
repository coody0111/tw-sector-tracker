...
- [ ] Step 1: 先用瀏覽器/`curl`/`requests` 手動打一次 TWSE 大盤指數 API，確認實際 endpoint 跟回應格式（json 還是 csv，欄位長怎樣）
- [ ] Step 2: 依照實測結果寫 `_parse_taiex_response()` 的測試（含正常回應跟擋頁兩種情境）
- [ ] Step 3: 跑測試確認 FAIL
- [ ] Step 4: 實作 `fetch_taiex_index()` + `_parse_taiex_response()`
- [ ] Step 5: 跑測試確認 PASS
- [ ] Step 6: 對真實 API 手動測一次，確認資料抓得到、格式跟預期一致
...
- [ ] Step 1: 新增 `TAIEX_HEAVYWEIGHTS: list[str]`，TAIEX 市值前 10-20 大成分股的 `stock_id`（台積電 2330、鴻海 2317、聯發科 2454、台達電 2308...等，實作時查證正確清單跟排序，不要憑記憶亂填）
- [ ] Step 2: 加註解說明這份清單需要定期（建議半年）review 更新，因為市值排名會變動
...
- [ ] Step 1: 寫 `calc_market_breadth` 測試（含 0 檔/全漲/全跌邊界情況）
- [ ] Step 2: 寫 `calc_capital_concentration` 測試（權值股領漲、非權值股領漲兩種情境都要）
- [ ] Step 3: 寫 `classify_market_regime` 測試（五級邊界值各測一次；指數落在「小漲」但廣度 <50% 時應該退回「持平」；集中度超過門檻時 `is_concentrated=True`）
- [ ] Step 4: 跑測試確認全部 FAIL
- [ ] Step 5: 實作三個函式
- [ ] Step 6: 跑測試確認全部 PASS
...
- [ ] Step 1: `run()` 裡呼叫 `fetch_taiex_index()`（比照現有 TWSE 呼叫的 try/except 模式：`TWSEBlockedError` 個別處理、當天無資料時 fallback 前一交易日，不要讓大盤指數抓取失敗擋掉整個每日流程）
- [ ] Step 2: 呼叫 `calc_market_breadth()`、`calc_capital_concentration()`、`classify_market_regime()`，組成 `market_regime` dict
- [ ] Step 3: 傳給 `generate_html(...)` 多一個 `market_regime=market_regime` 參數
- [ ] Step 4: 抓取失敗時的 fallback：`market_regime=None`，`html_generator.py` 那邊要處理這個區塊完全不顯示的情況（不要讓缺這個資料就讓整頁 crash）
...
- [ ] Step 1: 寫測試（含 `market_regime=None` 的防呆情境）
- [ ] Step 2: 跑測試確認 FAIL
- [ ] Step 3: 實作 `_market_regime_section()`，包含：五級標籤 + 對應顏色、集中度診斷（權值股/非權值股數字並排顯示）、對應筆記操作提示文字（設計文件裡的對照表）
- [ ] Step 4: 接進 `generate()` 主函式，放在最上方（族群排行榜之上）
- [ ] Step 5: 跑測試確認 PASS
- [ ] Step 6: 用合成資料跑一次 `generate()`，肉眼確認渲染出來的 HTML 視覺上合理
...
- [ ] Step 1: 全專案測試都過
- [ ] Step 2: 用真實 `data/screener.db` 跑一次 `python main.py`，確認 `docs/index.html` 正確產出新區塊，數字合理（可以跟當天財經新聞報的大盤漲跌對一下，抓明顯抓錯的情況）
- [ ] Step 3: 刻意製造 TAIEX 抓取失敗的情境（例如暫時改錯 URL），確認整頁不會 crash，只是這個區塊不顯示
- [ ] Step 4: 交給 Debugger 依 `debug-tasks.md` 慣例驗證：門檻數字合理性、`_esc()` 有沒有漏用、`main.py` 的 fallback 邏輯
