...
.stock-cards-wrap{display:grid;grid-template-columns:repeat(auto-fill,minmax(150px,1fr));gap:6px}
.stock-item{display:flex;align-items:baseline;gap:8px;border:1px solid var(--border);border-radius:5px;
  padding:8px 10px;background:var(--panel-3);cursor:pointer;transition:border-color .15s}
.stock-item:hover{border-color:var(--border-2)}
.stock-item:focus-visible{outline:2px solid var(--accent);outline-offset:1px}
.stock-item.no-data{opacity:.5;cursor:default}
.stock-item .si-id{font-family:var(--mono);color:var(--ink-3);font-size:.66rem}
.stock-item .si-name{font-family:var(--serif);font-weight:600;color:var(--ink);font-size:.82rem;flex:1;min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
.stock-item .si-price{font-family:var(--mono);font-size:.78rem;color:var(--ink-2)}
.stock-item .si-pct{font-family:var(--mono);font-size:.78rem;font-weight:700}
...
function renderStockListItem(s) {{
  ...
  if (s.no_data) {{
    return `<div class="stock-item no-data"><span class="si-id">${{sid}}</span><span class="si-name">${{escHtml(s.stock_name)}}</span><span class="si-pct">無行情</span></div>`;
  }}
  const color = s.change_pct >= 0 ? 'var(--up)' : 'var(--down)';
  const sign = s.change_pct >= 0 ? '+' : '';
  const arrow = s.change_pct > 0 ? '▲' : (s.change_pct < 0 ? '▼' : '─');
  return `<div class="stock-item" role="button" tabindex="0" onclick="openStockCard('${{sid}}')" `
    + `onkeydown="if(event.key==='Enter'||event.key===' '){{event.preventDefault();openStockCard('${{sid}}')}}">`
    + `<span class="si-id">${{sid}}</span><span class="si-name">${{escHtml(s.stock_name)}}</span>`
    + `<span class="si-price">${{fmtPrice(s.close)}}</span>`
    + `<span class="si-pct" style="color:${{color}}">${{arrow}} ${{sign}}${{s.change_pct.toFixed(2)}}%</span></div>`;
}}
