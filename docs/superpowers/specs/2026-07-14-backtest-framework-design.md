...
### 🔴 按大盤 regime 分段驗（重要，否則只驗到一個 regime）

**超額報酬只擋掉 beta，擋不掉 regime 偏差**：一個訊號在多頭/輪動市有 edge，不代表在空頭市也有（甚至可能相反）。若把所有歷史攪在一起算平均，會把某段行情的特性誤當通則。

- 每筆訊號依**訊號日 D 的大盤 regime** 分組（多頭 / 盤整 / 空頭），**各段各自算**勝率/超額/期望值。
- v1 regime 用**大盤等權指數的回看報酬**簡單分段（近 20 日 ≥ +3% → 多頭、≤ −3% → 空頭、其間 → 盤整）；之後可換成已做好的大盤分級儀表板 `classify_market_regime`（方向 + 資金集中度）。
- 這也呼應進貨分 spec 的 caveat：那邊「籌碼配角」結論來自單一 regime，回測分段才驗得出「籌碼在不同 regime 是否不同價值」。
