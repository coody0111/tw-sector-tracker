...
```text
週一至週五
09:00～15:00（TWSE 正式收盤 13:30，13:30~15:00 是延伸監控緩衝，見 §10.1 說明）
每 15 分鐘
```
...
2026-09-07 前持續時間是 4 小時 45 分（09:00~13:45），13:45~15:00 收盤摘要之間是空窗期；
拉長到 6 小時後這段也會繼續輪詢——`run_scheduled.py::is_market_hours()` 同步拉長到 15:00，
但 TWSE 正式收盤仍是 13:30（`_MARKET_CLOSE` 常數保留原值僅供語意參考），13:30 後的輪詢是
監控緩衝、不代表市場還在交易；intraday 模式本身冪等，訊號沒變不會重複發 Telegram 通知。
...
⚠️ `close` 模式沒有「今天已經跑過」的防重複機制，只要成功執行就會無條件送一次收盤通知——
目前不能單純多加幾個 15:00 之後的 close 觸發當保險，會變成重複收到收盤摘要。

### 10.3 執行限制

- 桌電關機時無法盤中執行
- 桌電睡眠時排程設定了 `-WakeToRun`（2026-09-07 起，見 `scripts/install_scheduler.ps1`），
  時間到會自動喚醒電腦執行；喚醒後若又進入睡眠、任務執行到一半仍可能被中止（2026-09-07
  實測發生過：13:17 盤中正常跑完，之後電腦睡眠超過 3 小時，16:38 補跑收盤時執行到一半
  被中止，沒有產生 HTML、沒有 push、沒有通知）
